...
В большинстве современных программ компьютер не считает непрерывно, а "ждет"
различных данных от сети, завершения операции ввода-вывода и т.д. Пока программа
ждет, процессор фактически простаивает. Традиционный способ решения проблемы -
создавать отдельные потоки или использовать коллбэки, что, однако, усложняет код и
дороже по ресурсам. Решение этой проблемы - корутины. Корутина - это функция, которая может
приостанавливать и возобновлять свое выполнение, сохраняя при этом свое
состояние. Проще говоря, корутина позволяет написать код так, как будто 
он выполняется последовательно, но на самом деле он работает асинхронно и эффективно. 
...
программирования в веб-серверах, высоконагруженных API, асинхронных клиентах баз данных и т.д.
Корутины делятся на два типа - *stackful* и *stackless*. Первые
